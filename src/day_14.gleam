...
pub fn move_robot(robot: Robot, seconds: Int, space: Space) -> Robot {
  ...
  let assert Ok(new_x) =
    int.modulo(robot.pos.x + { robot.vel.x * seconds }, width)
  let assert Ok(new_y) =
    int.modulo(robot.pos.y + { robot.vel.y * seconds }, height)
...
pub fn get_quadrants(robots: List(Robot), space: Space) {
  ...
  let is_in_upper_half = fn(robot: Robot) { robot.pos.y < mid_height }

  let is_in_lower_half = fn(robot: Robot) { robot.pos.y > mid_height }

  let is_in_left_half = fn(robot: Robot) { robot.pos.x < mid_width }

  let is_in_right_half = fn(robot: Robot) { robot.pos.x > mid_width }

  let quadrant_nw =
    robots
    |> list.filter(fn(robot) {
      is_in_upper_half(robot) && is_in_left_half(robot)
    })

  let quadrant_ne =
    robots
    |> list.filter(fn(robot) {
      is_in_upper_half(robot) && is_in_right_half(robot)
    })

  let quadrant_sw =
    robots
    |> list.filter(fn(robot) {
      is_in_lower_half(robot) && is_in_left_half(robot)
    })

  let quadrant_se =
    robots
    |> list.filter(fn(robot) {
      is_in_lower_half(robot) && is_in_right_half(robot)
    })
...
pub fn highest_row_count(robots: List(Robot)) {
  robots
  |> list.group(fn(robot) { robot.pos.y })
...
pub fn main() {
  ...
  |> list.map(fn(seconds) {
    let moved_robots =
      robots
      |> list.map(fn(robot) { move_robot(robot, seconds, space) })
