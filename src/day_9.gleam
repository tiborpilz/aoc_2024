  ...
  #(
    ...
    dict.Dict(
      ...
      #(
        Int,
        Int,
        // Keys of files (descending)
      ),
    ),
    dict.Dict(
      Int,
      Int,
      // File ID to start block and length
    ),
    // Free gap keys to their size
  )
  ...
  |> list.index_fold(
    ...
    fn(acc, curr, index) {
      ...
      let new_gaps = case is_gap {
        False -> gap_keys
        True ->
          case parsed_block_size == 0 {
            False ->
              list.append(
                gap_keys,
                list.range(key_offset, { key_offset + parsed_block_size } - 1),
              )
            True -> gap_keys
          }
      }
        ...
        False ->
          case parsed_block_size == 0 {
            False ->
              list.append(
                key_offset
                  |> list.range({ key_offset + parsed_block_size } - 1)
                  |> list.reverse,
                content_keys,
              )
            True -> gap_keys
      ...
      let new_blocks =
        list.repeat(block_value, parsed_block_size)
...
pub fn move_last_block_to_smallest_gap(blocks: Blocks) {
  ...
  #(
    new_block_content,
    list.append(gap_keys_rest, [last_content_key]),
    list.append(content_keys_rest, [smallest_gap_key]),
    files,
    gap_sizes,
  )
}
...
pub fn is_defragmented(blocks: Blocks) -> Bool {
  let assert [smallest_gap_key, ..] = blocks.1
  let assert [last_content_key, ..] = blocks.2
