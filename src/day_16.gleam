...
pub fn parse_map(input: List(List(String))) -> Map {
  input
  |> list.map(fn(row) { list.map(row, fn(char) { parse_tile(char) }) })
  |> grid.from_lists
}

pub fn stringify_map(map: Map) -> grid.Grid(String) {
  map
  |> dict.map_values(fn(_, tile) { tile_to_string(tile) })
}
...
pub fn heuristic_score(
  ...
  turn_cost
  + int.absolute_value(current_x - target_x)
  ...
  case dict.get(known_scores, current_position) {
    ...
    _ ->
      case dict.get(already_checked, current_position) {
        ...
        _ ->
          case current_position == target_position {
            ...
            False -> {
              ...
              list.fold(possible_steps, #(scores, known_scores), fn(acc, curr) {
                ...
                let updated_known_paths = case
                  new_scores
                  |> list.sort(fn(a, b) { int.compare(a, b) })
                  |> list.first
                {
                  Ok(result) ->
                    dict.insert(new_known_scores, position, [result])
                  _ -> new_known_scores
                }
...
pub fn get_scores(
  ...
  case counter % 100 {
    0 -> {
      ...
      io.print(
        "Current Position: "
        <> grid.position_to_string(current_position)
        <> "Current path length: "
        <> current_path |> list.length |> int.to_string,
      )
      io.print("\r")
      io.print("\r")
      ""
        ...
        False -> {
          let possible_steps =
            ...
            |> list.sort(fn(a, b) {
              ...
              let estimated_score_a =
                step_cost_a + heuristic_score(pos_a, target_position)
              let estimated_score_b =
                step_cost_b + heuristic_score(pos_b, target_position)
              int.compare(estimated_score_a, estimated_score_b)
            })
          // A Star babyyy
...
pub fn get_paths(
  ...
  case checked, is_target {
    Ok(True), _ -> paths
    _, True -> [
      #(current_score |> io.debug, [current_position, ..current_path]),
      ..paths
    ]
    _, _ -> {
      let possible_steps =
        ...
        |> list.sort(fn(a, b) {
          // A* babyyy
          int.compare(
            get_step_cost(current_direction, a.1),
            get_step_cost(current_direction, b.1),
          )
        })
...
pub fn print_path(path: Path, map: Map) {
  path
  |> list.fold(stringify_map(map), fn(acc, curr) { dict.insert(acc, curr, "X") })
  |> grid.pretty_print
...
pub fn main() {
  ...
  let scores =
    get_paths(map, start_position, None, [], 0, [], dict.new(), end_position)

  let assert Ok(best_score) =
    scores
    |> list.sort(fn(a, b) {
      let #(score_a, _) = a
      let #(score_b, _) = b
