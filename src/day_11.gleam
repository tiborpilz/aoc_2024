...
pub fn update_and_return(dict: dict.Dict(k, v), key: k, value: v) -> #(v, dict.Dict(k, v)) {
...
pub fn process_element(
  ...
  case dict.get(memoized, #(input, times)), times {
    ...
    |> list.fold(#([], memoized), fn (acc, curr) {
      ...
      let #(new_elements, new_memoized) = process_element(curr, times - 1, memoized)
      // #(list.append(new_elements, elements), new_memoized)
      #(list.append(new_elements, elements), dict.insert(new_memoized, #(input, times), new_elements))
      // #(list.append(processed_element, acc), dict.insert(#(input, times), process_element))
    })
  }
}

pub fn mutate_list(input: List(Int), times: Int, memoized: dict.Dict(#(Int, Int), List(Int))) -> List(Int) {
  let #(mutated_list, _) = input
  |> list.fold(#([], memoized), fn (acc, curr) {
    let #(acc_list, acc_memoized) = acc
    let #(new_elements, new_memoized) = process_element(curr, times, acc_memoized)
...
pub fn add_to_with_default(input: dict.Dict(Int, Int), key: Int, value: Int) -> dict.Dict(Int, Int) {
...
pub fn process_line_out_of_order(input: dict.Dict(Int, Int), count: Int) -> dict.Dict(Int, Int) {
  case count {
    0 -> input
    _ -> process_line_out_of_order(
      dict.fold(input, dict.new(), fn (acc, key, value) {
        mutate_element(key) |> list.fold(acc, fn (acc, curr) { add_to_with_default(acc, curr, value) })
    }), count - 1)
  }
}
...
pub fn list_to_dict(input: List(Int)) -> dict.Dict(Int, Int) {
  ...
  |> list.fold(dict.new(), fn (acc, curr) {
    add_to_with_default(acc, curr, 1)
  })
}
