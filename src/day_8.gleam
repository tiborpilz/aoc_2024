...
pub fn main() {
  let input_grid = "./data/day_8.txt"
  |> utils.read_lines
  |> list.map(fn (row) { string.split(row, "") })
  |> grid.from_lists

  let frequencies = input_grid
  ...
  |> list.map(fn (freq) {
    dict.filter(input_grid, fn(_, value) {
      value == freq
    })
    |> dict.to_list
  })
  ...
  let empty_antinode_grid = "." |> list.repeat(width) |> list.repeat(height) |> grid.from_lists

  let antenna_grid = antennas
  |> list.flatten
