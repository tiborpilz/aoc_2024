...
pub fn get_surrounding_entries(grid: grid.Grid(Int), position: #(Int, Int)) -> List(#(Int, Int)) {
...
pub fn grid_to_string(grid: grid.Grid(Int)) -> grid.Grid(String) {
  dict.map_values(grid, fn (_, value) {
    int.to_string(value)
  })
}
...
pub fn get_paths(
  ...
  |> list.filter(fn (surrounding_position) {
    let assert Ok(surrounding_elevation) = dict.get(grid, surrounding_position)
    surrounding_elevation == current_elevation + 1
  })

  case current_elevation {
    9 -> [[position, ..current_path], ..paths]
    _ -> case list.length(new_positions) {
      0 -> paths
      _ -> list.map(new_positions, fn (new_position) {
